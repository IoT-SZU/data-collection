...
import cn.ac.futurenet.data_collection.utils.WaveHeader;
...
public class AudioService {
    ...
    public void startRecord(final WaveView waveView) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                readAndWriteData(waveView);
            }
        }).start();
    }

    private void readAndWriteData(WaveView waveView) {
        // new一个byte数组用来存一些字节数据，大小为缓冲区大小
        byte[] audiodata = new byte[bufferSizeInBytes];
        short[] audio = new short[bufferSizeInBytes / 2];
            ...
            while (isRecording) {
                ...
                if (AudioRecord.ERROR_INVALID_OPERATION != count) {
                    for (int i = 0; i < bufferSizeInBytes; i += 2) {
                        audio[i / 2] = audiodata[i];
                        audio[i / 2] += audiodata[i + 1] << 8;
                    }
                    waveView.update(audio, bufferSizeInBytes / 2);
//                    fos.write(audiodata);
                }
            }
